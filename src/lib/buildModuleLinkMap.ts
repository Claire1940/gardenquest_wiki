...
const MODULE_FIELDS: Record<string, { field: string; nameKey: string }> = {
  gardenQuestReleaseAndPlayLink: { field: 'items', nameKey: 'value' },
...
const MODULE_EXTRA_KEYWORDS: Record<string, string[]> = {
  gardenQuestReleaseAndPlayLink: ['release date', 'roblox', 'discord', 'youtube', 'trailer', 'launch'],
...
function matchScore(queryText: string, article: ArticleWithType, extraKeywords?: string[]): number {
  ...
  // Exact phrase match in title (stripped of "Garden Quest")
  const strippedQuery = normalizedQuery.replace(/garden quests?\s*/g, '').trim()
  const strippedTitle = normalizedTitle.replace(/garden quests?\s*/g, '').trim()
